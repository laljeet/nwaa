# Authors and Citation

## Authors

- **Laljeet Sangha**. Author, maintainer.

## Citation

Source:
[`inst/CITATION`](https://github.com/laljeet/nwaa/blob/v0.1.2/inst/CITATION)

Sangha L (2026). *nwaa: An R interface to the USGS National Water
Availability Assessment Data Companion*.
[doi:10.5281/zenodo.19984099](https://doi.org/10.5281/zenodo.19984099).
R package version 0.1.2, <https://doi.org/10.5281/zenodo.19984099>.

    @Manual{,
      title = {nwaa: An R interface to the USGS National Water Availability Assessment Data Companion},
      author = {Laljeet Sangha},
      year = {2026},
      note = {R package version 0.1.2},
      doi = {10.5281/zenodo.19984099},
      url = {https://doi.org/10.5281/zenodo.19984099},
    }
