---
title: "Getting started with nwaa"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting started with nwaa}
...
The `nwaa` package provides an R interface to the USGS National Water Availability Assessment (NWAA) Data Companion web service ([https://water.usgs.gov/nwaa-data](https://water.usgs.gov/nwaa-data)). The service publishes model outputs at HUC12 spatial resolution across three families:
...
Every supported model and its variables live in a single tibble. The catalog is the source of truth for what models, variables, units, and time resolutions are valid:
...
To inspect the variables for one model, the simplest path is the helper for the Water Use family:

```{r catalog-vars-wu, eval = FALSE}
...
For non-Water-Use models, list-columns from the catalog give the same information:

```{r catalog-vars, eval = FALSE}
...
NWAA data are published at HUC12 resolution, but the service accepts queries at any HUC level, by lowercase 2-letter state abbreviation, or by 5-digit county FIPS code:
...
When `location_type` is `"statecd"` or `"countycd"`, the polygon is converted to a set of HUC12s server-side. The `intersection` argument controls strictness: `"overlap"` (default) includes any HUC12 that touches the polygon, `"envelop"` requires the HUC12 to be at least 98% inside.

## One example per family

The four family wrappers all share the same argument structure: pick a model, pick variables, pick a location, pick a date range. Each returns a tibble (or list/sf object for `format = "json"` or `"geojson"`).

### Water Use

```{r wu-example, eval = FALSE}
wu <- nwaa_water_use(
...
For monthly resolution, `start` and `end` use `"YYYY-MM"` format. For annual resolution, they use `"YYYY"`. The package picks the right format based on `time_res`.

## Temporal resolution

Water Use models support `monthly`, `annualcy` (calendar year), and `annualwy` (water year, October to September, labeled by ending year). Water Quantity and Integrated Water Availability models are published as monthly products only. Annual rollups for the latter can be computed client-side with standard tools:

```{r aggregate-pattern, eval = FALSE}
library(dplyr)
...
Every wrapper validates the request against the catalog before any network call. Invalid model IDs, variables that don't belong to the model, and unsupported `time_res` values fail fast with a readable message:

```{r validate-bad, eval = FALSE}
nwaa_water_use(
  model_id      = "wu-irrigation-wd",
...
Vector inputs are not yet supported on the `location_id` argument. To pull the same query across several counties, use `purrr::map_dfr()` or a `lapply()` plus `dplyr::bind_rows()` pattern. Native multi-location support is planned for a later release.

```{r multi-county, eval = FALSE}
library(purrr)
library(dplyr)
...
The Water Use family has the most models and the most-developed examples. See `vignette("water-use", package = "nwaa")` for a full walkthrough across all five Water Use models.
